...
window = turtle.Screen()  # get the canvas screen associated with the turtle world so we can process events
...
window.onclick(mouseClickEventHander) # register an event handler.  Here we're saying, whenever
...
window.onclick(rightMouseClickEventHander,2) # same as left click, except now register right-click (button 2)
